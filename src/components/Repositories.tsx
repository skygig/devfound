...
const Repositories = ({
  ...
  useEffect(() => {
    ...
    const newRepoList = repoListData.filter((repo) => {
      for (const skill of userSkills) {
        if (
          Object.keys(repo.languages).includes(skill) ||
          (repo.frameworks as string[])?.includes(skill) ||
          (repo.tools as string[])?.includes(skill)
        )
          return true;
      }
      return false;
  ...
  const handleSearch = (e: any) => {
    ...
    if (!toSearch) {
      ...
      const filterRepo = repoList.filter((repo) =>
        repo.title.includes(toSearch)
      );
  ...
  const handlePage = (action: number) => {
    ...
    if ((action === -1 && page === 0) || (action == 1 && page >= maxPage))
      return;
    setPage(page + action);
  };
    ...
    >
      ...
      <div className={styles.reposContainer}>
        <div className={styles.reposHead}>
          <p>
            Showing {page * 12} - {Math.min(page * 12 + 12, repoList.length)} of{" "}
            {repoList.length} repositories
          </p>
          <div>
            <p className={styles.pageControl} onClick={() => handlePage(-1)}>
              {"<"}
            </p>
